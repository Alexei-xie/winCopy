...
namespace WinCopy {
    public class SnippetEditor : Form {
        TextBox title = new TextBox(), group = new TextBox(), body = new TextBox();
        ...
        public SnippetEditor(Clip clip) {
            ...
            foreach (int h in new[] { 28, 40, 28, 40, 28 }) grid.RowStyles.Add(new RowStyle(SizeType.Absolute, h)); grid.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); grid.RowStyles.Add(new RowStyle(SizeType.AutoSize)); Controls.Add(grid);
            grid.Controls.Add(new Label { Text = "片段名称", AutoSize = true }); title.Dock = DockStyle.Fill; title.MaxLength = 100; title.Text = clip == null ? "" : clip.Snippet ? clip.Title : clip.Text.Substring(0, Math.Min(24, clip.Text.Length)).Replace("\r", " ").Replace("\n", " "); grid.Controls.Add(title);
            grid.Controls.Add(new Label { Text = "分组", AutoSize = true }); group.Dock = DockStyle.Fill; group.MaxLength = 80; group.Text = clip == null ? "常用" : clip.Group; grid.Controls.Add(group);
            grid.Controls.Add(new Label { Text = "内容", AutoSize = true }); body.Dock = DockStyle.Fill; body.Multiline = true; body.AcceptsReturn = true; body.ScrollBars = ScrollBars.Vertical; body.MaxLength = 1000000; body.Text = clip == null ? "" : clip.Text; grid.Controls.Add(body);
            var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, WrapContents = false, FlowDirection = FlowDirection.RightToLeft, Margin = Padding.Empty, Padding = new Padding(0, 12, 0, 4) }; var ok = new Button { Text = "保存", AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, MinimumSize = new Size(96, 36), Padding = new Padding(12, 5, 12, 5), Margin = new Padding(6, 0, 0, 0) }; ok.Click += delegate { if (TitleValue.Length == 0 || GroupValue.Length == 0 || BodyValue.Trim().Length == 0) { MessageBox.Show("请填写名称、分组和内容。"); return; } DialogResult = DialogResult.OK; }; buttons.Controls.Add(ok); var cancel = new Button { Text = "取消", DialogResult = DialogResult.Cancel, AutoSize = true, AutoSizeMode = AutoSizeMode.GrowAndShrink, MinimumSize = new Size(96, 36), Padding = new Padding(12, 5, 12, 5), Margin = new Padding(6, 0, 0, 0) }; buttons.Controls.Add(cancel); CancelButton = cancel; grid.Controls.Add(buttons);
        }
    }
    public partial class SettingsDialog : Form {
        ...
        CheckBox paste = new CheckBox { Text = "选择内容后自动粘贴到之前的应用" }, images = new CheckBox { Text = "记录图片（单张最多 8 MB）" }, remember = new CheckBox { Text = "退出后保留历史（收藏和片段始终保留）" }, startup = new CheckBox { Text = "登录 Windows 时在托盘启动" };
        TextBox excluded = new TextBox();
        public Action ExportAction, ImportAction;
        ...
        void AddRow(FlowLayoutPanel flow, string caption, Control input) { var row = new Panel { Width = 478, Height = 39 }; row.Controls.Add(new Label { Text = caption, AutoSize = true, Location = new Point(0, 5) }); input.Location = new Point(338, 2); input.Width = 130; row.Controls.Add(input); flow.Controls.Add(row); }
    }
}
