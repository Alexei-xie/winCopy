...
namespace WinCopy {
    public class SnippetEditor : Form {
        ...
        public SnippetEditor(Clip clip, IEnumerable<string> existingGroups) {
            ...
            grid.Controls.Add(new Label { Text = "分组", AutoSize = true }); group.Dock = DockStyle.Fill; group.MaxLength = 80; group.DropDownStyle = ComboBoxStyle.DropDown; group.AutoCompleteMode = AutoCompleteMode.SuggestAppend; group.AutoCompleteSource = AutoCompleteSource.ListItems; group.AccessibleName = "分组，可选择已有分组或输入新名称"; foreach(var name in existingGroups.Where(x => !String.IsNullOrWhiteSpace(x)).Distinct()) group.Items.Add(name); group.Text = clip == null ? "常用" : clip.Group; grid.Controls.Add(Design.Input(group));
