...
namespace WinCopy {
    public partial class MainWindow {
        ...
        void BuildUi() {
            ...
            var context = new ContextMenuStrip(); context.Items.Add("复制", null, delegate { UseSelected(false, true); }); context.Items.Add("以纯文本粘贴", null, delegate { UseSelected(true, false); }); context.Items.Add("收藏 / 取消收藏", null, delegate { TogglePin(); }); context.Items.Add("编辑 / 保存为片段", null, delegate { if (Selected != null) EditSnippet(Selected); }); context.Items.Add("模板 / 独立快捷键…",null,delegate{ConfigureSelectedSnippet();}); context.Items.Add("删除", null, delegate { DeleteSelected(); }); TrackMenu(context); list.ContextMenuStrip = context;
