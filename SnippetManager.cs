...
namespace WinCopy {
    ...
    public class SnippetManager : Form {
        public readonly Database Working; public Func<string,bool> ShortcutAvailable;
        ...
        public SnippetManager(Database source){
            ...
            Add(tools,"新建",()=>Edit(null));Add(tools,"编辑",()=>{if(Selected.Length==1)Edit(Selected[0]);});Add(tools,"复制片段",Duplicate);Add(tools,"模板 / 快捷键",Options);Add(tools,"重命名分组",Rename);Add(tools,"移动选中",MoveSelected);Add(tools,"删除选中",Delete);Add(tools,"上移",()=>Shift(-1));Add(tools,"下移",()=>Shift(1));
        ...
        void ReloadRows(string[] selected){rows.BeginUpdate();rows.Items.Clear();foreach(var c in Working.OrderedSnippets().Where(x=>x.Group==CurrentGroup)){var item=new ListViewItem(c.Title+(c.IsTemplate?" [模板]":"")+(String.IsNullOrEmpty(c.ShortcutKey)?"":" [快捷键 "+c.ShortcutKey+"]")){Tag=c};item.SubItems.Add(SearchPresentation.Excerpt(c.Text,""));rows.Items.Add(item);item.Selected=selected.Contains(c.Id);}rows.EndUpdate();}
        ...
        void Options(){if(Selected.Length!=1)return;var clip=Selected[0];using(var dialog=new SnippetOptionsDialog(clip))if(dialog.ShowDialog(this)==DialogResult.OK)try{if(dialog.IsTemplate)SnippetTemplates.Variables(clip.Text);if(dialog.ShortcutKey.Length>0&&Working.Items.Any(x=>x!=clip&&x.Snippet&&x.ShortcutKey==dialog.ShortcutKey))throw new InvalidOperationException("该快捷键已分配给另一个片段。");if(ShortcutAvailable!=null&&!ShortcutAvailable(dialog.ShortcutKey))throw new InvalidOperationException("快捷键已被其他应用占用。");clip.IsTemplate=dialog.IsTemplate;clip.ShortcutKey=dialog.ShortcutKey;ReloadRows(new[]{clip.Id});}catch(Exception ex){MessageBox.Show(this,ex.Message,"无法保存");}}
        void Duplicate(){foreach(var c in Selected)Working.Items.Add(new Clip{Snippet=true,Group=c.Group,Title=c.Title.Substring(0,Math.Min(95,c.Title.Length))+" 副本",Text=c.Text,IsTemplate=c.IsTemplate});ReloadRows();}
