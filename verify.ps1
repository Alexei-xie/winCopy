...
$exe = Join-Path $PSScriptRoot 'dist\winCopy.exe'
$result = Start-Process -FilePath $exe -ArgumentList '--self-test' -Wait -PassThru -WindowStyle Hidden
Get-Content -LiteralPath (Join-Path $PSScriptRoot 'dist\self-test-result.txt')
...
try {
    ...
    if ($main.Width -ne 440 -or $main.FormBorderStyle -ne "None" -or $main.ShowInTaskbar) { throw "Popup geometry regression" }
    ...
    $bitmap.Save((Join-Path $PSScriptRoot 'dist\preview.png'))
    $bitmap.Dispose()
    ...
    try {
        ...
        $bitmap.Save((Join-Path $PSScriptRoot 'dist\settings-preview.png'))
        $bitmap.Dispose()
