...
namespace WinCopy {
    public class GroupPicker : RoundedPanel, IMessageFilter {
        ...
        public void Open(bool filter){if(IsDisposed||!Visible||IsOpen)return;if(popup==null){popup=new GroupPopup(Choose);popup.Caption=OptionsCaption;owner=FindForm();if(owner!=null){owner.Deactivate+=OwnerChanged;owner.LocationChanged+=OwnerChanged;owner.Resize+=OwnerChanged;}}
    ...
    internal class GroupPopup : Form {
        ...
        protected override void OnPaint(PaintEventArgs e){Design.Surface(e.Graphics,new Rectangle(0,0,Width-1,Height-1),Color.White,12,Design.Border);
            using(var small=new Font(Font.FontFamily,8.5f))TextRenderer.DrawText(e.Graphics,Caption??(values.Count==0?"无匹配分组，输入名称后保存即可创建":"选择已有分组 · 可输入新名称，滚轮查看更多"),small,new Rectangle(14,8,Width-28,22),Design.Muted,TextFormatFlags.EndEllipsis|TextFormatFlags.SingleLine);
            for(int i=offset;i<values.Count;i++){int y=32+(i-offset)*40;if(y+38>Height)break;bool active=i==selected;if(active)Design.Surface(e.Graphics,new Rectangle(7,y,Width-14,36),Design.Accent,8,Color.Empty);TextRenderer.DrawText(e.Graphics,values[i],Font,new Rectangle(18,y+6,Width-54,26),active?Color.White:Design.Ink,TextFormatFlags.EndEllipsis|TextFormatFlags.NoPrefix);if(active)TextRenderer.DrawText(e.Graphics,"✓",Font,new Rectangle(Width-34,y+6,24,26),Color.White);}}
