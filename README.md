...
需要 .NET Framework 4.8（较新 Windows 10 / Windows 11 已包含），不需要 Node.js 或浏览器运行时。安装包未进行代码签名，Windows 可能显示未知发布者提示。升级前请右键系统托盘图标退出旧版。卸载保留 `%LOCALAPPDATA%\winCopy` 中的数据。
...
在常用片段上右键 →「模板 / 独立快捷键…」，或在片段管理中选中一条片段 →「模板 / 快捷键」配置。

- 模板默认关闭，旧片段保持原文。开启后，`{{date}}` 展开为当前日期，`{{time}}` 为当前时间，`{{datetime}}` 为日期与时间（本地时间）。例如：`{{date}}，你好 {{姓名}}！`。
- `{{姓名}}` 等自定义变量会在复制/粘贴前弹出填写窗口。最多 20 个不同变量，同名变量只填一次；输入值按原文插入，不递归解析。取消不修改剪贴板，填写值不会写回片段。展开后最多 100 万字符。
- 为常用片段分配 `Ctrl + Alt + Shift + A–Z / 0–9`，可直接调用，不必搜索。支持清除绑定；保存时检查重复分配与系统占用。片段管理窗口中的配置在点击「保存更改」后生效。若启动时被其他程序占用，会提示重新设置。
- 复制片段保留模板模式，但不复制快捷键。加密备份保留模板模式；合并恢复新增片段时清除导入的快捷键，避免与当前电脑冲突。现有片段的快捷键不变。明文 XML 不保存模板开关或快捷键，导入后需手动配置。

## 片段管理、备份与按时间清理

主面板更多菜单 →「管理片段 / 分组」，或设置 →「数据」进入片段管理。支持分组重命名（同名时确认合并）、Ctrl/Shift 多选、批量移动/删除、复制片段、组内拖动排序及上移/下移。点击「保存更改」后一次性写入，取消不保存；写入失败保留当前数据。空分组自动移除，组内顺序会保存在本地并用于片段 XML 导出。

设置分为「常规、记录、存储、数据、关于」，保存/取消按钮固定在底部。

- **创建备份**：设置 → 数据 → 创建加密备份，保存 `.wcbak` 并设置至少 8 位密码。备份包含当前历史、收藏、片段和分组排序，包括仅本次会话保留的记录。文件历史仅保存路径，不包含文件实体。备份上限 128 MB、10000 条记录。
- **恢复备份**：在本机或另一台电脑输入密码，确认后合并并去重，保留当前快捷键、存储路径等设置。普通历史继续遵守当前保留天数、数量和负载限制，过期记录可能不会保留。关闭历史持久化时，恢复的普通历史也仅保留在本次会话。恢复写入成功后才替换当前数据。
- **密码与格式**：备份使用 PBKDF2-SHA256 派生密钥、AES-256-CBC 加密和 HMAC-SHA256 完整性校验。密码不会保存，遗失无法恢复。运行中的本地 history.dat 仍使用 Windows 账户加密；请用 `.wcbak` 跨电脑迁移，不要直接复制 history.dat。片段 XML 是明文交换格式，仅包含片段。
- **按时间清理**：更多菜单或设置 → 记录 → 按时间清理历史，选择日期后预览清理数量；确认后删除早于该日期 00:00 的未收藏普通历史。收藏、片段和系统剪贴板保留。清理即时生效，不可撤销。
