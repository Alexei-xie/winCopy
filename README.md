...
搜索结果会高亮标题、正文摘要、来源或片段分组中的匹配词，并显示结果数量；正文深处的匹配内容会提取为摘要。图片历史直接显示缩略图，最多缓存 64 张 64×64 小图，移除记录时释放对应缓存。

普通历史删除后，底部提供 10 秒「撤销」入口，也可在列表获得焦点时按 Ctrl + Z。只保留最近一次普通删除，关闭程序后不保留；新复制了相同内容时，撤销不会生成重复记录。清空历史和已确认的收藏/片段删除不提供撤销。记录恢复仍遵守历史数量、保留天数和负载限制。

复制与收藏操作会在顶部短暂反馈，收藏按钮同步显示当前状态。设置的「存储位置」区域显示当前 history.dat 文件大小、记录/图片数量和图片压缩数据量，可手动刷新；该统计不是进程内存占用，不包含临时文件和迁移前的旧目录副本。选择新目录但尚未保存时，统计仍对应当前使用的目录。

## 图片放大预览

选中图片历史后，点击底部图片缩略图打开大图窗口。支持滚轮或 ＋/− 按钮缩放、拖动查看局部、「适应窗口」和「原始大小」。双击图片切换适应窗口与原始大小，按 Esc 关闭。查看窗口独立保留图片，新增历史不会改变正在查看的内容。
